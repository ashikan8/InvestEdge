...
TRADING_DAYS_PER_YEAR = 252.0  # annualization factor (analysis is always daily)
...
# Data utilities — analysis uses daily bars over a multi-year window. Intraday
# data was deliberately removed: annualizing per-bar stats from a short intraday
# window produces wildly unstable figures (a routine 2-month drawdown on 30-min
# bars extrapolates to absurd numbers like -79%/yr), and intraday autocorrelation
# distorts volatility. Daily/5y is the standard, stable basis for mean-variance.
DATA_INTERVAL = "1d"
DATA_PERIOD = "5y"
...
def get_price_data(tickers: List[str]) -> pd.DataFrame:
    ...
    try:
        raw = yf.download(tickers, interval=DATA_INTERVAL, period=DATA_PERIOD, progress=False, auto_adjust=False)
        data = raw["Adj Close"]
...
def fetch_spy_returns() -> Optional[pd.Series]:
    """Download SPY once and compute its daily returns, used as the market (CAPM) factor."""
    try:
        spy = yf.download("SPY", interval=DATA_INTERVAL, period=DATA_PERIOD, progress=False, auto_adjust=False)["Adj Close"]
...
def _naive_index(idx: pd.Index) -> pd.Index:
    """Strip timezone so SPY (possibly tz-aware) and the asset panel align by date."""
    return idx.tz_localize(None) if getattr(idx, "tz", None) is not None else idx

def market_relative_mu(returns: pd.DataFrame, spy_rets: Optional[pd.Series]) -> Optional[np.ndarray]:
    """CAPM-style per-bar expected return for each asset: rf + beta_i * (E[mkt] - rf).

    Unlike a flat market-mean prior (which shrinks every asset toward the same
    number), this scales the market premium by each asset's beta, so high-beta
    names get a higher implied return and low-beta names a lower one. Betas are
    estimated over the overlapping dates and Blume-adjusted toward 1.0 to tame
    estimation noise on short windows. Returns None if SPY data is unusable, so
    the caller can fall back to the plain EW mean.
    """
    if spy_rets is None or len(spy_rets) == 0:
        return None
    spy = spy_rets.copy()
    spy.index = _naive_index(spy.index)
    aligned = spy.reindex(_naive_index(returns.index))   # SPY return on each asset date (NaN if missing)
    mask = aligned.notna().values
    if mask.sum() < 2:
        return None
    spy_v = aligned.values[mask]
    var_spy = float(np.var(spy_v))
    if var_spy <= 0:
        return None
    mkt_mean = float(spy_v.mean())
    rf_bar = RISK_FREE_ANNUAL / TRADING_DAYS_PER_YEAR
    R = returns.values[mask]                              # asset returns on the same dates, shape (m, k)
    spy_dev = spy_v - mkt_mean
    asset_dev = R - R.mean(axis=0, keepdims=True)
    beta = (asset_dev * spy_dev[:, None]).mean(axis=0) / var_spy   # cov(asset, mkt)/var(mkt)
    beta = 0.67 * beta + 0.33                              # Blume adjustment: shrink toward beta = 1
    return rf_bar + beta * (mkt_mean - rf_bar)
...
def robust_covariance(returns: pd.DataFrame) -> np.ndarray:
    ...
    if _HAS_SKLEARN:
        try:
            ...
            pass
    # np.cov returns a 0-d scalar for a single column; force a 2-D matrix so
    # downstream `w @ Sigma @ w` works for single-asset portfolios.
    return np.atleast_2d(np.cov(returns.values, rowvar=False))

_SPY_RETS_UNSET = object()  # sentinel distinguishing "fetch it for me" from an explicit, already-failed None

def _blended_mu(returns: pd.DataFrame, spy_rets=_SPY_RETS_UNSET) -> np.ndarray:
    """Per-bar expected returns: BLEND_ALPHA * EWMean(asset) + (1-alpha) * CAPM_i.

    The CAPM term is a beta-adjusted market prior (see market_relative_mu): it
    pulls each asset's mean toward its market-implied return, reducing the noise
    in raw historical means without flattening every asset to the same number.
    ...
    important inside the walk-forward backtest. Omit it (the _SPY_RETS_UNSET
    sentinel) to fetch fresh.
    """
    mu_ew = exp_weighted_mean(returns, halflife=EW_HALFLIFE).values
    if spy_rets is _SPY_RETS_UNSET:
        spy_rets = fetch_spy_returns()
    capm = market_relative_mu(returns, spy_rets)
    if capm is None:
        return mu_ew
    return (BLEND_ALPHA * mu_ew) + ((1.0 - BLEND_ALPHA) * capm)
...
def optimize_portfolio(
    ...
    mu = _blended_mu(returns, spy_rets)
...
def backtest_static(prices: pd.DataFrame, weights: np.ndarray) -> Dict[str, float]:
    ...
    return _backtest_stats(port, TRADING_DAYS_PER_YEAR)
...
def backtest_walkforward(
    ...
    bpyr = TRADING_DAYS_PER_YEAR
    rebalance_every = max(5, int(round(bpyr / 12)))  # ~monthly rebalancing

    spy_rets = fetch_spy_returns()  # fetch once, reuse across every rebalance window's fit
    ...
    while i < n:
        ...
        weights, _ = optimize_portfolio(
            train, strategy=strategy, max_weight=max_weight,
...
def optimize_api():
    ...
    if not np.isfinite(portfolio_value) or portfolio_value <= 0:
        return jsonify({"error": "portfolio_value must be a positive, finite number."}), 400
    ...
    prices_df = get_price_data(tickers)
    if prices_df.empty:
